...
namespace RG
{
	...
	{
		FuncPtr<GLRenderer>* windowResizeCallback = new FuncPtr<GLRenderer>(this, &GLRenderer::OnWindowResizeCallback);
		EventManager::AddCallback(EventType::OnWindowResize, windowResizeCallback);

		FuncPtr<GLRenderer>* RenderEntitiesCallback = new FuncPtr<GLRenderer>(this, &GLRenderer::RenderEntitiesImpl);
		EventManager::AddCallback(EventType::OnRender, RenderEntitiesCallback);
	}
	...
	void GLRenderer::RenderEntitiesImpl(Event* event)
	{
		...
		for (int i = 0; i < e->vec->GetLength(); i++)
		{
			...
			TextureComponent* t = e->vec[0][i]->GetComponent<TextureComponent>();
			if (t != nullptr)
				t->Bind();
